...
export default function ProjectPageClient({ id }: Props) {
  ...
  if (!project) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center py-12 px-4 min-h-[70vh]">
          ...
          <CardContent className="p-8 flex flex-col items-center text-center space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full pt-2">
              <Button asChild className="w-full sm:flex-1" size="lg">
                <Link href="/projects">
                  {t('projects.notFound.backProjects')}
                </Link>
              </Button>
          ...
          {projects.map((p) => (
            <div key={p.id} className="w-full rounded-lg h-full">
              <Link href={`/projects/${p.id}`} className="w-full rounded-lg overflow-hidden border bg-gradient-to-br from-primary/10 via-secondary/10 to-accent/10 dark:bg-card dark:bg-none shadow-[0_8px_20px_rgba(80,150,217,0.16)] hover:shadow-[0_16px_34px_rgba(80,150,217,0.24)] hover:scale-[1.015] transition-all duration-200 h-full flex flex-row md:flex-col items-center md:items-stretch">
                <div className="relative h-20 w-28 sm:h-24 sm:w-36 md:h-32 md:w-full bg-muted/10 flex-shrink-0">
                  {p.image ? (
                ...
                <div className="p-3 text-left md:text-center flex-1 flex items-center justify-start md:justify-center">
                  <h3 className="text-sm font-semibold text-foreground dark:text-white line-clamp-2">{t(p.titleKey)}</h3>
                </div>
              </Link>
